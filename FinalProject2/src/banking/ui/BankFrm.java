...
//		setDefaultCloseOperation(javax.swing.JFrame.DO_NOTHING_ON_CLOSE);
		...
		/*
		 * /Add buttons on the pane /for Adding personal account, Adding company
...
//		 mouthHoverbar(reportButton);
	...
	public void accountsActionPerformed(ActionEvent event) {
		...
		if (!accnum.isEmpty()) {
			...
			// assuming saving and checking account are in the same table in
			// database
	...
	void JButtonPerAC_actionPerformed(java.awt.event.ActionEvent event) {
		...
		if (newaccount) {
			...
			//
			// Customer customer =
			// CustomerFactory.createCustomer(CustomerType.INDIVIDUAL);
			// //customer.setId(id);
		...
		if (selection >= 0) {
			...
			if (newamount < 0) {
				JOptionPane.showMessageDialog(JButton_Withdraw,
						"You don't have enough balance in your account " + accnr, "Warning: low balance",
